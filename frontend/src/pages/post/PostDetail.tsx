  ...
  const getSalaryRangeText = (salaryRange: string): string => {
    switch (salaryRange) {
      case "under25":
        return "2,500만원 미만";
      case "25to35":
        return "2,500-3,500만원";
      case "35to50":
        return "3,500-5,000만원";
      case "over50":
        return "5,000만원 이상";
      default:
        return salaryRange || "협의";
    }
  };
  ...
  const renderDeveloperTypeTags = (developerTypes: string) => {
    ...
    try {
      ...
      return types.map((type: string, index: number) => (
        <span key={index} className="tag dev-type-tag">
          {type}
        </span>
      ));
    } catch (e) {
  ...
  if (!jobPosting) {
    return (
      <div className="post-detail-main-wrap">
        ...
        <div className="container">
          <main className="main-content">
            <div className="empty-content">공고 정보를 찾을 수 없습니다.</div>
          </main>
        </div>
      </div>
    );
        ...
        <main className="main-content">
          ...
          <section className="job-header">
            <div className="job-meta">
              ...
              {jobPosting.industry || "업종 정보없음"} • 직원{" "}
              {jobPosting.empCount || "정보없음"}명
            </div>
              ...
              {userInfo ? (
                isOwner ? (
                  ...
                  <>
                    ...
                    <button
                      className="btn-applicant_details"
                      onClick={handleViewApplicants}
                    >
                      지원자 보기
                    </button>
                    <button
                      className="btn-applicant_scout"
                      onClick={handleViewScout}
                    >
                      스카웃 하기
                    </button>
                  </>
                  ...
                  // 다른 기업의 공고를 보는 경우
                  <button className="btn-scout">매칭 제안하기</button>
                ) : (
                  // 개발자 사용자인 경우
                  <button
                    className={`btn-apply ${
                      jobPosting.isApplied ? "btn-applied" : ""
                    }`}
                    onClick={handleApply}
            ...
            <div className="section-content">
              <div className="info-tags">
                ...
                <div className="info-tag-item">
                  ...
                  <span className="info-tag-value">
                    {getWorkTypeText(jobPosting.workType)}
                  </span>
                </div>
                <div className="info-tag-item">
                  <span className="info-tag-label">경력 요구사항</span>
                  <span className="info-tag-value">
                    {getExperienceLevelText(jobPosting.experienceLevel)}
                  </span>
                </div>
                <div className="info-tag-item">
                  <span className="info-tag-label">급여 범위</span>
                  <span className="info-tag-value">
                    {getSalaryRangeText(jobPosting.salaryRange)}
                  </span>
                </div>
                <div className="info-tag-item">
                  <span className="info-tag-label">등록일</span>
                  <span className="info-tag-value">
                    {formatDate(jobPosting.postedAt)}
                  </span>
                </div>
                <div className="info-tag-item">
